...
// ==================================================================
// Math Solve Instruction — সম্পূর্ণ নতুন, স্বনির্ভর ব্লক। উপরের কোনো রুট/কোড
// স্পর্শ করা হয়নি। নিজস্ব আলাদা টেবিল ব্যবহার করে (ai_settings/settings.js
// এর কিছুই ছোঁয়া হয়নি) — প্রথম কলেই টেবিল নিজে থেকে তৈরি হয়ে যায়।
//
// উদ্দেশ্য: এডমিন Math Solve ট্যাব থেকে যে instruction লেখে, সেটা
// globally সব কাস্টমার/সব ডিভাইসে পৌঁছাতে হবে। আগে এটা শুধু browser
// localStorage-এ সেভ হতো (per-browser), তাই অন্য কাস্টমারের কাছে
// পৌঁছাতো না — এই টেবিলটাই সেই মূল bug-এর fix।
// ==================================================================
const DEFAULT_MATH_INSTRUCTION =
`গণিতের যেকোনো সমস্যা রোবটের মতো একলাফে চূড়ান্ত উত্তর দিয়ে দিও না। বরং একজন মানুষ যেভাবে খাতায় ধাপে ধাপে অংক কষে, ঠিক সেভাবেই সমাধান করো:
- প্রতিটি ধাপ আলাদা আলাদা লাইনে লিখবে।
- প্রতিটি লাইনে হিসাবের দুই পাশ সমান (=) চিহ্ন দিয়ে সাজিয়ে দেখাবে, যাতে আগের ধাপ থেকে পরের ধাপে কীভাবে গেল সেটা স্পষ্ট বোঝা যায়।
- মাঝপথে কোনো ধাপ বাদ দিয়ে লাফিয়ে যাবে না, প্রতিটি হিসাব দেখাবে।
- সবগুলো ধাপ দেখানোর পর সবশেষে চূড়ান্ত উত্তরটি আলাদা লাইনে স্পষ্টভাবে উল্লেখ করবে।`;

let __mathInstructionTableReady = false;
async function ensureMathInstructionTable() {
  if (__mathInstructionTableReady) return;
  await query(`
    CREATE TABLE IF NOT EXISTS math_instruction (
      id INT PRIMARY KEY DEFAULT 1,
      content TEXT NOT NULL DEFAULT '',
      updated_at TIMESTAMP NOT NULL DEFAULT now(),
      CHECK (id = 1)
  ...
  `);
  await query(
    `INSERT INTO math_instruction (id, content) VALUES (1, $1) ON CONFLICT (id) DO NOTHING`,
    [DEFAULT_MATH_INSTRUCTION]
  );
  __mathInstructionTableReady = true;
}

// chat.js থেকে ইমপোর্ট করে কাস্টমার-facing GET রুটে ব্যবহার হবে (যেকোনো লগইন করা
// কাস্টমার, যেকোনো ডিভাইস থেকে, সবসময় একই global instruction পাবে)
export async function getMathInstruction() {
  await ensureMathInstructionTable();
  const row = await queryOne('SELECT content FROM math_instruction WHERE id = 1');
  return { content: (row && row.content) || DEFAULT_MATH_INSTRUCTION };
}

// ---- Admin: Math Solve instruction টেমপ্লেট দেখা/সেভ করা (super admin only) ----
router.get('/math-instruction', requireSuperAdmin, async (req, res) => {
  try {
    res.json(await getMathInstruction());
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Could not load the math solve instruction.' });
  }
});

router.post('/math-instruction', requireSuperAdmin, async (req, res) => {
  try {
    const { content } = req.body || {};
    await ensureMathInstructionTable();
    const finalContent =
      typeof content === 'string' && content.trim() ? content.slice(0, 20000) : DEFAULT_MATH_INSTRUCTION;
    await query('UPDATE math_instruction SET content = $1, updated_at = now() WHERE id = 1', [finalContent]);
    res.json({ ok: true });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: 'Could not save the math solve instruction.' });
  }
});
