...
// Math Solve ফিচারের জন্য — admin.js-এর নতুন self-contained ব্লক থেকে import
import { getMathInstruction } from './admin.js';
...
/* ============================================================================================
   FIX — নতুন যোগ করা রুট: Math Solve global instruction (কাস্টমার-facing, read-only)
   ফ্রন্টএন্ডের Math Solve ফিচারের আসল উদ্দেশ্য পূরণ করতে (এডমিনের লেখা instruction সব
   কাস্টমার/সব ডিভাইসে পৌঁছানো), যেকোনো লগইন করা কাস্টমার এই রুট কল করে সবসময় admin.js-এর
   math_instruction টেবিলে সেভ করা সর্বশেষ instruction টেক্সট পাবে — ব্রাউজার/ডিভাইস আলাদা
   হলেও ফলাফল একই থাকবে। এটা requireUser-এর পরে বসানো (উপরের router.use(requireUser)
   অনুযায়ী), তাই টোকেন লাগবে, কিন্তু আর কোনো বিশেষ প্ল্যান/কোটা/ব্লক শর্ত নেই।
   ============================================================================================ */
router.get('/math-instruction', async (req, res) => {
  try {
    const data = await getMathInstruction();
